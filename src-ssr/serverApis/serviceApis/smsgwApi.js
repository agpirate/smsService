...
async function create_employee(reqData,isxFile=true){
  ...
  try {
    ...
    var empData =""
    if(isxFile){
      empData = reqData//await compute_Profile(reqData)
...
router.post(modelIName, async (req, res) => {
  ...
  await create_employee(reqData).then((modelData) => {
    //console.log(reqData,'Posting Profile Req')
...
async function update_employee(reqData,findBy,isxFile=true){

  var ErrorDepth = "Computing Processing.."
  try {
    var _incomingData =""
    if(isxFile){
      _incomingData = reqData//await compute_Profile(reqData)
...
router.put(modelIName, async (req, res) => {
  ...
  var _uploading = nul.includes(reqParams['upload']) ? false : reqParams['upload']
  let [findBy, returnWat, limits] = await _postputParams(reqParams);
  ...
  console.log('updating..',uploadf_,'___________',_uploading)
    ...
    if(uploadf_){//upload_['files']
      ...
      multer(getFileOptions()).single(uploadf_)  (req, res, async (err) => {
        ...
        console.log('updating..zzzzzzzzzzzzzzzzzzzzzTTTTTT',typeof uploadf_)
        if (req.fileValidationError) {
          return res.status(404).json("validation_Erro :-"+req.fileValidationError);
         } else if (nul.includes(req.file)) {
         return res.status(404).json("req.file(s) is null."+ req.headers);
      ...
      let fileMeta ={}
      if(filetoUpload){fileMeta = await extractFileMeta(filetoUpload)}
      ...
      if (typeof reqData == 'object' && Object.keys(reqData).length == 0) {
        ...
        return await update_employee(reqData,findBy,false).then((modelData) => {
          if (modelData.status == 200) {
            res.set(resHeader);
            res.cookie("access_token", "sessionsID", { httpOnly: true });
            return res.status(200).json(modelData['data']);
      ...
      console.log('updating..zzzzzzzzzzzzzzzzzzzzz',uploadf_,'___________',reqData)

     return await update_employee(reqData,findBy).then((modelData) => {
...
router.get(modelIName, async (req, res) => {
  ...
  try {
    ...
    ////console.log(reqParams,findBy,'profile searching....',modelData)
